...
#else
#ifdef ICEMAN3D
      WRITE(X_DIR_PIN, invert_dir[X_AXIS]);
#else
      WRITE(X_DIR_PIN, INVERT_X_DIR);
#endif
#endif
...
#ifdef ICEMAN3D
      WRITE(Y_DIR_PIN, invert_dir[Y_AXIS]);
#else
...
#ifdef ICEMAN3D
          bool x_min_endstop = (READ(X_MIN_PIN) != endstops_inverting[X_AXIS]);
#else
...
#ifdef ICEMAN3D
        bool y_min_endstop = (READ(Y_MIN_PIN) != endstops_inverting[Y_AXIS]);
#else
...
#ifdef ICEMAN3D
      WRITE(Z_DIR_PIN, invert_dir[Z_AXIS]);
#else
...
#endif
#ifdef ICEMAN3D
          bool z_min_endstop = (READ(Z_MIN_PIN) != endstops_inverting[Z_AXIS]);
#else
...
ISR(TIMER0_COMPA_vect)
{
  ...
  for (unsigned char i = 0; i < 4; i++) {
    if (e_steps[0] != 0) {
      ...
      if (e_steps[0] < 0) {
#ifdef ICEMAN3D
        WRITE(E0_DIR_PIN, invert_dir[E_AXIS]);
#else
    ...
    case X_AXIS:
      {
        ...
        //setup new step
#ifdef ICEMAN3D
        WRITE(X_DIR_PIN, (invert_dir[X_AXIS])^direction);
#else
    ...
    case Y_AXIS:
      {
        ...
        //setup new step
#ifdef ICEMAN3D
        WRITE(Y_DIR_PIN, (invert_dir[Y_AXIS])^direction);
#else
